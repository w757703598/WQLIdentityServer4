<template>
  <div class="login">
    ...
    <p class="copyright">
      © 2019 TEST有限公司. All rigthts reserved.
    </p>
  </div>
</template>
<script>
export default {
  data() {
    return {
      ...
      loginForm: {
        ...
        remeber: true,
        grant_type: "password",
        scope: "AntennaKnowbaseApi openid profile IdentityServer",
  ...
  methods: {
    ScanTimeout() {
      ...
      let _this = this;
      if (thisTime - this.clicktime > this.outTime) {
        console.info(thisTime - _this.clicktime);
        _this.$router.push({ path: "/login" });
        window.clearInterval(this.timefunction);
      }
    },
    async submit() {
